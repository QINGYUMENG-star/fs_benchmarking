...
def RF_fs(data, label, model_params, training_params,name, folder_name, logger, causal_variants=None):
    ...
    logger.info(f" RF_fs start at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    logger.info("execute Random Forest...")
    try:
        if task_type in ['binary', 'multiclass']:
            ...
            y_train = y_train.reshape(-1).astype(float)
        ...
        feature_scores = feature_importances[feature_indices]
        
        logger.info(f"Random Forest finished. The total number of features: {len(feature_indices)}")
        logger.info(f"Top 10 feature importances: {feature_scores[:10]}")
        logger.info(f"Top 10 feature indices: {feature_indices[:10]}")

        np.save(os.path.join(folder_name, f'RF_{name}_weights.npy'), 
                feature_importances)
        np.save(os.path.join(folder_name, f'RF_{name}_idx.npy'), 
                feature_indices)        

        # result_file = os.path.join(folder_name, f"rf_features.npz")
        # np.savez(
        #     result_file,
        ...
        logger.info(f"Random Forest finished. Elapsed time: {format_time(elapsed_time)} ({elapsed_time:.2f} seconds)")

        return feature_indices
        
    except Exception as e:
        logger.error(f"Random Forest feature selection failed: {str(e)}")
        logger.error(traceback.format_exc())
        raise e


def RF_fs_with_evaluation(data, label, model_params, training_params, name, 
                       features_selected, digits, folder_name, 
                       logger, feature_prediction, n_iters=20, 
                       feature_step=500, n_folds=3):

    total_start_time = time.time()
    logger.info(f" RF_with_evaluation start at  {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    task_type = model_params.get('task_type', 'binary')
    ...
    for iter in range(start_iter, n_iters):
        ...
        try:
            ...
            try:
                ...
                if task_type in ['binary','multiclass']:
                    y_train_rf = y_train
                    if y_train_rf.ndim > 1 and y_train_rf.shape[1] > 1:
                        y_train_rf = np.argmax(y_train_rf, axis=1)
                    y_train_rf = y_train_rf.astype(int).ravel()
                    rf = RandomForestClassifier(
                        max_features='sqrt',
                        random_state=seeds[iter], n_jobs=-1
                    )
                else:
                    y_train_rf = y_train.reshape(-1).astype(float)
                    rf = RandomForestRegressor(
                        max_features='sqrt',
                        random_state=seeds[iter], n_jobs=-1
                    )
                ...
                logger.info(f"successfully completed Random Forest feature ranking, top 10 feature importances: {feature_importances[feature_indices[:10]]}")
                logger.info(f"top 10 feature indices: {feature_indices[:10]}")
            except Exception as e:
                logger.error(f"Random Forest feature selection error: {str(e)}")
                logger.error(traceback.format_exc())
                raise e
            ...
            rf_results_file = os.path.join(iter_folder, f'feature_ranking_iter_{iter}.npz')
            np.savez(rf_results_file, 
                    feature_indices=feature_indices, 
                    feature_importances=feature_importances,
                    iter=iter, 
                    seed=seeds[iter])
            logger.info(f"feature ranking save to: {rf_results_file}")
            ...
            for step in feature_steps_range:
                ...
                if num_features > len(feature_indices):
                    logger.warning(f"requested features {num_features} exceed available features {len(feature_indices)}, skipping")
                    continue
    ...
    save_path = os.path.join(folder_name, f'RF_{name}_results.npz')
    np.savez(save_path, **final_results)

    # timestamp_save_path = os.path.join(
    #     folder_name, 
    #     f'RF_{name}_results_{final_results["timestamp"]}.npz'
    # )
