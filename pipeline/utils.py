...
#     job_root = os.path.abspath(os.path.join(out_dir, os.pardir))
#     log_path = os.path.join(job_root, "pipeline.log")
...
def setup_logger(out_dir: str, method: str, do_parameter_search: int, use_evaluation: int):

    job_root = os.path.abspath(os.path.join(out_dir, os.pardir))
    log_name = f"pipeline_{method}_{do_parameter_search}_{use_evaluation}.log"
    log_path = os.path.join(job_root, log_name)
...
def _is_probably_classification(y, logger):
    ...
    - int/bool 类型 → 当作分类
    - float 但唯一值不多 → 当作分类
    只是为了在 split 时决定要不要 stratify，真正任务类型还是后面
    auto_detect_and_fix_labels 再精细判断。
    """
    y = np.asarray(y)
    n = y.shape[0]
    uniq = np.unique(y)
...
def _split_train_test(X, y, train_ratio, seed, logger, classification=None):
    """
    用 sklearn.model_selection.train_test_split 来划分：
    - 如果 classification=True，则使用 stratify=y
    - 否则不 stratify
    """
    n = X.shape[0]
    if n != len(y):
    ...
    # 如果几乎不留 test，或者样本太少，就直接复制 train，当作“无独立测试集”
    if n < 2 or test_size <= 0:
        ...
        return X, X.copy(), y, y.copy(), False
    ...
    try:
        ...
        # 比如某个类别样本太少导致 stratify 失败，就 fallback 到非 stratify
        logger.warning(
        ...
        classification = False  # 实际上没有 stratify 了

    logger.info(
        f"[Split] Done: n_train={x_train.shape[0]}, n_test={x_test.shape[0]}, "
        f"stratified={classification}"
    )
