...
class EvaluationNet(nn.Module):
    """
    A lightweight evaluation network: two hidden layers + optional output head
    for multiclass/binary/regression.
    - task_type: 'binary' | 'multiclass' | 'regression'
    - num_classes: number of classes for multiclass (>=2)
    ...
    def __init__(self, input_size, task_type, num_classes=None, dropout_prob=0.2):
        ...
        if task_type == 'multiclass':
            ...
            head = nn.Linear(32, num_classes)  # output logits
        else:
            head = nn.Linear(32, 1)            # binary/regression output (1-d)

        self.network = nn.Sequential(hidden, head)

    def forward(self, x):
        return self.network(x)  # directly output logits/values; apply activation externally as needed


def _get_criterion(task_type: str):
    """
    Select the loss criterion based on task type:
...
def train_fold_return_best_valloss(
    ...
    """
    Used only during hyperparameter tuning/cross-validation: train and return
    the minimum validation loss for this parameter combination (val_loss,
    lower is better).

    Note: this stage does NOT compute reporting metrics such as AUC/F1/R.
    """
    ...
    # Return the minimum val_loss for this parameter combination (used for hyperparameter comparison)
...
def compute_report_metrics(
    ...
    include_raw: bool = False,            # newly added: whether to include raw y/pred in metrics
    proba_from: str = "logits",           # "logits" or "probs", used for multiclass/binary
):
    """
    Reporting metrics for the final model on the test set.
    - binary: AUC, Accuracy, F1, Precision, Recall
    - multiclass: Accuracy, F1-macro, F1-weighted, per-class P/R/F1, per-class AUROC (macro/weighted)
    - regression: MSE, MAE, R^2, Pearson r

    Args
    ----
    task_type: 'binary' | 'multiclass' | 'regression'
    y_true: one-dimensional numpy array
    logits_or_preds:
        - binary: (N,) or (N,1) logits (if proba_from='logits') or probabilities (if proba_from='probs')
        - multiclass: (N,C) logits (or probabilities depending on proba_from)
        - regression: (N,) or (N,1) predictions
    num_classes: number of classes for multiclass
    include_raw: whether to include raw y/pred values in metrics for later analysis
    proba_from: indicates whether logits_or_preds are 'logits' or 'probs' (affects sigmoid/softmax)
    """
    ...
    if task_type == 'binary':
        ...
        try:
            ...
            # AUROC may fail when a class is too imbalanced or missing; ignore
            pass
