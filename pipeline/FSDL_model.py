...
def FSDL(data,label, folder_name, training_params, model_params,name, method, digits,logger):
    """
...
def FSDL_with_evaluation(data, label, logger, model_params, training_params, name, 
    ...
    for iter in range(start_iter, n_iters):
        ...
        try:
            ...
            if current_training_params.get('do_parameter_search', False):
                ...
                logger.info(
                    f"current fixed FSDL architecture before Optuna: n_layers={current_model_params['n_layers']}, "
                ...
                if isinstance(results_dict, dict) and results_dict.get('best_architecture') is not None:
                    best_architecture = results_dict['best_architecture']
                    logger.info(
                        f"best FSDL architecture from Optuna: n_layers={best_architecture.get('n_layers')}, "
            ...
            for file in os.listdir(attribution_dir):
                if file.endswith('.npy') and method in file:
                    ...
                    break
            logger.info(f"Feature scores file found: {feature_scores_file}")
    ...
    final_results = {
        ...
        'timestamp': datetime.datetime.now().strftime('%Y%m%d_%H%M%S'),
        'per_step_report_metrics': np.array(all_report_metrics, dtype=object)  # newly added
    }
...
def optuna_search_FSDL(analyzer, data_o, method, model_class, model_params, training_params, folder_name, logger):
    ...
    def objective(trial):
        ...
        hidden_dims = [
            max(4, int(X.shape[1] / max(1, int(d))))
        ...
        if use_cv:
            ...
            if task_type in ('binary', 'multiclass'):
                if isinstance(y, torch.Tensor):
                    label_np = y.detach().cpu().numpy().ravel()
                else:
                    label_np = np.asarray(y).ravel()
        ...
        for fold, (train_idx, val_idx) in enumerate(split_iter):
            ...
            logger.info(f" Trial {trial_id}, Fold {fold+1}/{n_eval_splits} completed - Time: {format_time(fold_time)}")
    ...
    study = optuna.create_study(
        ...
        storage=storage,
        study_name=f"FSDL_{method}_shrink_ratio_v3",
        load_if_exists=True
    )
    ...
    logger.info(
        f"Running {n_trials} Optuna optimization trials with {n_jobs} parallel workers "
        f"(use_cv={use_cv})"
    )
    try:
    ...
    if 'n_layers' in best_params:
        ...
        final_model_params['digits_list'] = best_digits_list
        final_model_params['hidden_dims'] = [
            max(4, int(X.shape[1] / max(1, fallback_start_digit if i == 0 else int(round(fallback_start_digit * (fallback_shrink_ratio ** i))))))
            for i in range(fallback_n_layers)
        ]
        final_model_params['hidden_size'] = final_model_params['hidden_dims']
        ...
        final_model_params['hidden_size'] = resolve_hidden_sizes(final_model_params)
        final_model_params['hidden_dims'] = final_model_params['hidden_size']
        final_model_params['n_layers'] = int(final_model_params.get('n_layers', len(final_model_params['hidden_size'])))
    ...
    with open(os.path.join(folder_name, 'best_params.txt'), 'w') as f:
        ...
        f.write(f"Search time: {format_time(search_time)} ({search_time:.2f} sec)\n")
        f.write(f"Final model training time: {format_time(final_model_time)} ({final_model_time:.2f} sec)\n")
    ...
    return {
        ...
        'final_model_params': copy.deepcopy(final_model_params),
        'best_architecture': {
...
def fsdl_single_fold_train(X_train, y_train, X_val, y_val, model_class, model_params, training_params, folder_path, logger, eval_metric):
    ...
    for epoch in range(training_params['num_epochs']):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                y = batch['label']
                if task_type in ('binary', 'multiclass'):
                    y = y.long().to(device)        # use long for classification, shape=(N,)
                else:
                    y = y.float().view(-1,1).to(device)
    ...
    final_score = best_score if best_score is not None else (val_losses[-1] if eval_metric == 'loss' else val_metrics[-1].get(eval_metric, val_losses[-1]))
    return {
        'model': model,
...
# Helper function for resolving hidden sizes
def resolve_hidden_sizes(model_params):
    input_size = int(model_params['input_size'])

    if 'hidden_dims' in model_params and model_params['hidden_dims'] is not None:
        hidden_sizes = [max(4, int(h)) for h in model_params['hidden_dims']]
        if len(hidden_sizes) == 0:
            raise ValueError("model_params['hidden_dims'] must not be empty")
        expected_n_layers = max(2, int(model_params.get('n_layers', len(hidden_sizes))))
        if len(hidden_sizes) != expected_n_layers:
            raise ValueError(
                f"Length mismatch: n_layers={expected_n_layers}, but hidden_dims has {len(hidden_sizes)} entries"
            )
        return hidden_sizes
    ...
    if 'hidden_size' in model_params and model_params['hidden_size'] is not None:
        hidden_size = model_params['hidden_size']
        if isinstance(hidden_size, (list, tuple)):
            hidden_sizes = [max(4, int(h)) for h in hidden_size]
            expected_n_layers = max(2, int(model_params.get('n_layers', len(hidden_sizes))))
            if len(hidden_sizes) != expected_n_layers:
                raise ValueError(
                    f"Length mismatch: n_layers={expected_n_layers}, but hidden_size has {len(hidden_sizes)} entries"
                )
            return hidden_sizes
        hidden_size = max(4, int(hidden_size))
        n_layers = max(2, int(model_params.get('n_layers', 2)))
        return [hidden_size for _ in range(n_layers)]
    ...
    if 'digits' in model_params and model_params['digits'] is not None:
        ...
        hidden_size = max(4, input_size // digit)
        n_layers = max(2, int(model_params.get('n_layers', 2)))
        return [hidden_size for _ in range(n_layers)]

    raise ValueError("Unable to resolve hidden sizes from model_params")


# Refactored MLP baseline model
class Mlp_baseline(nn.Module):
    ...
    def __init__(self, feature_num, hidden_size, task_type='binary',
        ...
        if isinstance(hidden_size, int):
            ...
            raise ValueError("hidden_size must be an int, list, or tuple")

        if len(hidden_sizes) < 2:
            raise ValueError(f"hidden_size must contain at least 2 layers, got {len(hidden_sizes)}")
        hidden_sizes = [max(4, int(h)) for h in hidden_sizes]

        act_class = get_activation(activation)
        layers = []
        prev_dim = feature_num
        for h in hidden_sizes:
            layers.append(nn.Linear(prev_dim, h))
            layers.append(nn.BatchNorm1d(h))
            layers.append(copy.deepcopy(act_class))
...
# Refactored LRP-friendly MLP model
class Mlp_baseline_LRP(nn.Module):
    ...
    def __init__(self, feature_num, hidden_size,
        ...
        for h in hidden_sizes:
            layers.append(nn.Linear(prev_dim, h))
            layers.append(copy.deepcopy(act_cls))
            layers.append(nn.Dropout(dropout_rate))
            prev_dim = h
        self.hidden_layers = nn.Sequential(*layers)
...
class FeatureAttributionAnalyzer:
    ...
    def analyze_features(self, method_name, model_class, model_params, training_params):
        ...
        model_params['hidden_size'] = resolve_hidden_sizes(model_params)
        model_params['hidden_dims'] = model_params['hidden_size']
        model_params['n_layers'] = int(model_params.get('n_layers', len(model_params['hidden_size'])))
